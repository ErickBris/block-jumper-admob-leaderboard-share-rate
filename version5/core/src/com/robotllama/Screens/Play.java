...
public class Play implements Screen, InputProcessor{
	...
	public void render(float delta) { 
		...
		if(queueComplete() && actionsEmpty()){
			...
			case COUNTDOWN:
				...
				// kill ads
				if(!COUNTDOWNTRIGGERED)countDown();
				break;
		...
		if(hero.getoffScreen() && GAMESTATE != Gamestate.GAMEOVER){
			hero.reset();
			...
			if(queueComplete()){
				...
				timer.scheduleAtFixedRate(new TimerTask() {
					...
					public void run() {
						...
						Main.imageHandler.countdownText.setString(String.valueOf(seconds));
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		if(GAMESTATE == Gamestate.RUNNING)hero.jump();
		return false;
	}
